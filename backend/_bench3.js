...
const out = [];
function log(m){ out.push(m); fs.writeFileSync('/tmp/bench3.out', out.join('\n')+'\n'); }
(async () => {
  await connectDB();
  await new Promise(r=>setTimeout(r,1500));
  const vdb = getVoterDb();
  const docs = await vdb.collection('voters_all').aggregate([{ $sample: { size: 800 } }, { $project: { EPIC_NO: 1 } }]).toArray();
  ...
  async function run(n) {
    ...
    log(`concurrency ${String(n).padStart(4)} | wall ${String(total).padStart(6)}ms | throughput ${(n/(total/1000)).toFixed(0).padStart(6)}/s | p50 ${pct(lat,50)}ms | p95 ${pct(lat,95)}ms`);
  }
  for (const n of [50,100,200,500]) { await run(n); }
  log('DONE');
  process.exit(0);
